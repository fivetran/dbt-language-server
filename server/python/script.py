...
if sys.argv[1] == "--version":
    ...
    def new_get_columns_in_relation(self, relation: BaseRelation) -> List[BaseColumn]:
        ...
        conn.request("GET", "/macro?name=get_columns_in_relation&db=" +
                     db + "&schema=" + schema + "&table=" + table)
        response = conn.getresponse()
        result = response.read().decode()
        if result not in ["SOURCE", "NOT_FOUND"]:
            data = json.loads(result)
            bigquery_columns = [self.Column.create(
                column[0], column[1]) for column in data]
    ...
    def new_register_adapter(self, config: AdapterRequiredConfig) -> None:
        ...
        if credentials_type in ["bigquery", "snowflake"]:
            ...
            adapter.get_columns_in_relation = new_get_columns_in_relation.__get__(
                adapter)
    ...
    cli_args = [no_stats, "--no-use-colors"] + \
        sys.argv[3:] + ["--profiles-dir", profiles_dir]
