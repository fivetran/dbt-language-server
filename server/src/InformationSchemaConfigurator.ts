    ...
    ['jobs_by_user', 'jobs_by_project', 'jobs_by_folder', 'jobs_by_organization'].forEach(tableName => {
      InformationSchemaConfigurator.INFORMATION_SCHEMA_COLUMNS.set(tableName, [
        ...
        this.createColumnDefinition('job_stages', 'record', [
          ...
          this.createColumnDefinition('wait_ratio_avg', 'number'),
          this.createColumnDefinition('wait_ms_avg', 'string'),
          this.createColumnDefinition('wait_ratio_max', 'number'),
          this.createColumnDefinition('wait_ms_max', 'string'),
          this.createColumnDefinition('read_ratio_avg', 'number'),
          this.createColumnDefinition('read_ms_max', 'string'),
          this.createColumnDefinition('compute_ratio_avg', 'number'),
          this.createColumnDefinition('compute_ms_avg', 'string'),
          this.createColumnDefinition('compute_ratio_max', 'number'),
          this.createColumnDefinition('compute_ms_max', 'string'),
          this.createColumnDefinition('write_ratio_avg', 'number'),
          this.createColumnDefinition('write_ms_avg', 'string'),
          this.createColumnDefinition('write_ratio_max', 'number'),
        ...
        this.createColumnDefinition('dml_statistics', 'record', [
          this.createColumnDefinition('inserted_row_count', 'number'),
          this.createColumnDefinition('deleted_row_count', 'number'),
          this.createColumnDefinition('updated_row_count', 'number'),
        ]),
  ...
  addInformationSchemaTableColumns(tableName: string, informationSchemaCatalog: SimpleCatalogProto): void {
    ...
    if (tableDefinition && !informationSchemaCatalog.table?.find(t => t.name === tableName)) {
      ...
      tableDefinition.forEach(columnDefinition =>
        NewZetaSqlWrapper.addColumn(
          table,
          NewZetaSqlWrapper.createSimpleColumn(columnDefinition.name, NewZetaSqlWrapper.createType(columnDefinition)),
        ),
      );
    }
  }
