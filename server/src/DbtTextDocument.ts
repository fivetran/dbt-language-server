...
import { Diagnostic, DiagnosticSeverity, DidChangeTextDocumentParams, Hover, HoverParams, Position, Range, TextDocumentItem, _Connection } from 'vscode-languageserver';
...
export class DbtTextDocument {
    ...
    async onHover(hoverParams: HoverParams): Promise<Hover | null> {
        ...
        if (!hint) {
            const column = this.ast?.resolvedStatement?.resolvedQueryStmtNode?.query?.resolvedProjectScanNode?.inputScan?.resolvedFilterScanNode?.inputScan?.resolvedTableScanNode?.parent?.columnList?.find(c => c.name === text);
            if (column) {
