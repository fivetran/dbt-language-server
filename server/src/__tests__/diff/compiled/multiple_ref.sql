...
from `project-name-100`.`staging`.`opportunities_level_0_source`
left join `project-name-100`.`transforms`.`opportunities_level_1_quote_line_item_aggregation` using(opportunity_id)
left join `project-name-100`.`transforms`.`opportunities_level_1_bookings` using(opportunity_id)
left join `project-name-100`.`transforms`.`opportunities_level_1_stage_dates` using(opportunity_id)
left join `project-name-100`.`transforms`.`opportunities_level_1_stage_dates_technical` using(opportunity_id)
left join `project-name-100`.`transforms`.`opportunities_level_1_activity` using(opportunity_id)
